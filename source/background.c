...
int background_w_fld(
  ...
  switch (pba->fluid_equation_of_state) { /** <--- Modified by Artola, M. 2026 */
  case CLP:
    *w_fld = pba->w0_fld + pba->wa_fld * (1. - a);
    break;
  case SQRT:
    *w_fld = pba->w0_fld + pba->wa_fld * (1. - a)/pow(1. + pow((1. - a), 2.*pba->n_sigmoid), 1./(2.*pba->n_sigmoid));
    break;
  case TANH:
    *w_fld = pba->w0_fld + pba->wa_fld * pba->n_sigmoid * tanh((1. - a) / pba->n_sigmoid);
    break;
  case EDE:
  ...
  case CLP:
    *dw_over_da_fld = - pba->wa_fld;
    break;
  case SQRT:
    *dw_over_da_fld = -pba->wa_fld / pow(1. + pow((1.-a),2.*pba->n_sigmoid), 1.+1./(2.*pba->n_sigmoid));
    break;
  case TANH:
    *dw_over_da_fld = -pba->wa_fld / cosh((1.-a)/pba->n_sigmoid) / cosh((1.-a)/pba->n_sigmoid);
    break;
  case EDE:
  ...
  case CLP:
    *integral_fld = 3.*((1.+pba->w0_fld+pba->wa_fld)*log(1./a) + pba->wa_fld*(a-1.));
    break;
  case SQRT: /** <--- Modified by Artola, M. 2026 */
    *integral_fld = 0.;
    break;
  case TANH:
    *integral_fld = 0.;
...
/* kernel function for numerical integration of rho_fld <--- Modified by Artola, M. 2026 */
static int background_fld_integrand(
                                    double loga,
                                    double * y,
                                    double * dy,
                                    void * parameters_and_workspace,
                                    ErrorMsg error_message) {
  struct background * pba = parameters_and_workspace;
  double w_fld, dw_over_da_fld, integral_fld;
  double a = exp(loga);

  /* background_w_fld returns w(a) for the active parameterization */
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_) { /
    ...
    /* Numerical integration <--- Modified by Artola, M. 2026 */
    if ((pba->fluid_equation_of_state == SQRT) || (pba->fluid_equation_of_state == TANH)) {
      struct generic_integrator_workspace gi;
      double y_start[1];  // We only have 1 variable to integrate

      class_call(initialize_generic_integrator(1, &gi),
                 pba->error_message,
                 pba->error_message);

      y_start[0] = 0.0; // integral starts at 0

      class_call(generic_integrator(background_fld_integrand,
                                    log(a),
                                    0.0, // log(1) = 0
                                    y_start,
                                    pba, // parameters
                                    ppr->tol_background_integration, // eps
                                    ppr->smallest_allowed_variation, // hmin
                                    &gi),
                 gi.error_message,
                 pba->error_message);
      
      integral_fld = y_start[0];
